...
bot.start(async (ctx) => {
  try {
    await ctx.replyWithHTML(
      `Для вашего удобства мы разработали систему получения краткой, но емкой информации по часто задаваемым вопросам <b>(FAQ)</b> Для навигации используйте кнопку <b>Menu</b>`
    );
...
bot.command("team", async (ctx) => {
  try {
    ...
    await ctx.replyWithHTML(
      "<b>Гиряев Сергей Геннадиевич</b> основатель и руководитель клиники."
    );
    ...
    await ctx.replyWithHTML(
      "<b>Россов Богдан Витальевич</b> Врач-стоматолог ортопед-терапевт."
    );
    ...
    await ctx.replyWithHTML(
      "<b>Кадзаева Анна Андреевна</b> Врач-стоматолог эстетист."
    );
    ...
    await ctx.replyWithHTML(
      "<b>Ягубова Анна Михайловна</b> Врач-стоматолог ортодонт."
    );
    ...
    await ctx.replyWithHTML(
      "<b>Агзямова Гулия Гамилевна</b> наш администратор и ваш бессменный помощник по любым вопросам."
    );
  } catch (e) {
    console.error(e);
  }
...
bot.command("contacts", async (ctx) => {
  try {
    ...
    await ctx.replyWithHTML(
      `<a href="https://instagram.com/dr_gsg?igshid=YmMyMTA2M2Y=">Instagram</a>`
    );
...
bot.command("appointment", async (ctx) => {
  try {
    ...
    if (user != undefined) {
      ...
      await ctx.replyWithHTML(
        "Напишите пожалуйста ваше <b>Имя, Фамилию и номер телефона.</b> Наш администратор скоро свяжется с вами."
      );
      await bot.on("text", (ctx) =>
        ctx.telegram.sendMessage(-1001716780767, ctx.message.text)
      );
    }
  } catch (e) {
    console.error(e);
